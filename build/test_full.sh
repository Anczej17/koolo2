#!/bin/bash
# ROP_READ=full — every read through rmod. Zero external RPM is the goal.
set -e
cd "$(dirname "$0")"
...
CLAUDE_MODE=1 SKIP_ANTIDEBUG=1 STEALTH_READ=0 ROP_READ=full CLAUDE_READ_TRACE=1 ./app.exe &
sleep 3
...
echo "IN_GAME area=$AREA port=$PORT"

# Wait for presenter READY
READY=0
...
echo "PRESENTER READY — ROP_READ=full active"

echo "=== stats BEFORE ==="
curl -s -m 3 "http://127.0.0.1:$PORT/debug/read-trace-stats?character=Blizzard" 2>&1
echo

# Trigger 10 GetData ticks
for k in $(seq 1 10); do
  curl -s -m 10 "http://127.0.0.1:$PORT/debug/gamestate?character=Blizzard" > /dev/null 2>&1
  sleep 0.5
done

echo "=== stats AFTER 10 ticks ==="
curl -s -m 3 "http://127.0.0.1:$PORT/debug/read-trace-stats?character=Blizzard" 2>&1
echo
