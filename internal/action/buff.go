...
func BuffIfRequired() {
	...
	// Reset timer when in town (so buff triggers when leaving town)
	if inTown {
		ctx.LastBuffAt = time.Time{} // Reset to zero
		ctx.WasInTown = true
		return
	}

	// Detect leaving town - force buff on first time outside town
	justLeftTown := ctx.WasInTown && !inTown
	if justLeftTown {
		ctx.Logger.Debug("Left town - forcing buff sequence")
		Buff()
		// Only clear WasInTown after successful buff
	...
	// Normal rebuff check (every 30s)
	if !IsRebuffRequired() {
		return
	}

	// Safety: Don't buff if too many monsters nearby (only for rebuff, not initial buff)
	closeMonsters := 0
...
func Buff() {
	...
	ctx.BuffInProgress = true

	// Ensure we clear the flag when done
	defer func() {
		ctx.BuffInProgress = false
	}()
	...
	if !ctx.LastBuffAt.IsZero() && time.Since(ctx.LastBuffAt) < buffCooldown {
		return
	}

	// Set LastBuffAt at END of successful buff (not beginning)
	defer func() {
...
func castPreCTABuffs() {
	...
	for _, buff := range preBuffs {
		if _, found := ctx.Data.KeyBindings.KeyBindingForSkill(buff); found {
			...
			castBuffWithBestWeapon(buff, expectedState)
		}
	}
}
...
func castBarbarianWarcries() {
	...
	for _, warcry := range orderedWarcries {
		...
		castBuffWithBestWeapon(warcry, expectedState)
	}
}
...
// castBuffWithRetry ensures the buff skill is on the right mouse button,
...
func castBuffWithRetry(buffSkill skill.ID, expectedState state.State) {
	...
	for attempt := 0; attempt <= maxCastRetries; attempt++ {
		...
		if ctx.Data.PlayerUnit.RightSkill != buffSkill {
			ctx.Logger.Debug("Skill not on right button, skipping cast attempt",
				slog.String("skill", skillName))
			continue
		}
