...
// WalkToUnit represents packet 0x02 (WalkToUnit)
// Moves character to specified unit at walking speed
//
...
// NewWalkToUnit creates packet 0x02 for walking to a unit
...
func NewWalkToUnit(unitType int, unitId data.UnitID, originX, originY uint16) *WalkToUnit {
	return &WalkToUnit{
		PacketID: 0x02,
...
// GetPayload converts the WalkToUnit struct to bytes (214 bytes with padding)
// CRITICAL: Buffer must be 214 bytes, not 13!
func (p *WalkToUnit) GetPayload() []byte {
