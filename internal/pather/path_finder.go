...
func (pf *PathFinder) GetPath(to data.Position) (Path, int, bool) {
		...
		for _, n := range pf.data.NPCs {
			...
			for dy := -2; dy <= 2; dy++ {
				for dx := -2; dx <= 2; dx++ {
					...
					if towerY >= 0 && towerY < grid.Height && towerX >= 0 && towerX < grid.Width {
	...
	if pf.data.PlayerUnit.Area != area.ArcaneSanctuary && pf.data.CanTeleport() && found && len(path) > 1 {
		path = filterInvalidTeleportSteps(grid, path)
	}
...
func (pf *PathFinder) GetPathFrom(from, to data.Position) (Path, int, bool) {
	...
	// Ensure every point along the path is reachable
...
// filterInvalidTeleportSteps removes or replaces short teleports (<= 3 tiles) that target invalid terrain
func filterInvalidTeleportSteps(grid *game.Grid, path Path) Path {
...
// ensurePathReachability validates that every consecutive pair of points is at most
// MaxWaypointDistance tiles away (Chebyshev). Inserts intermediate waypoints if needed.
	...
	for i := 1; i < len(path); i++ {
		...
		if dist > MaxWaypointDistance {
			...
			for current != curr {
				...
				nextToTargetDx := utils.Abs(curr.X - nextWaypoint.X)
				nextToTargetDy := utils.Abs(curr.Y - nextWaypoint.Y)
