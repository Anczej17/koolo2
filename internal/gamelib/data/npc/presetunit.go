...
import (
	"local/internal/svc/internal/gamelib/data/monplace"
	"local/internal/svc/internal/gamelib/data/superunique"
)

type PresetUnit struct {
	PresetID int // Constructed by appending monstats.txt + superuniques.txt + monplace.txt to match map object npc id (relies on 1.13c map data).
}
