@echo off
REM Headless Ghidra runner: locate D2R input poll function via mouseXY xrefs.
...
set "D2R_REPORT=%REPO%\logs\AUDIT_D_input_poll.md"
...
python -m pyghidra.ghidra_launch --install-dir "%GHIDRA%" ghidra.app.util.headless.AnalyzeHeadless ^
    ...
    -noanalysis ^
    -postScript ghidra_re_input_poll.py ^
