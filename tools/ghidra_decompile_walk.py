...
#@category D2R
#
# Headless Ghidra script for request_walk RE.
#
# Loads the d2r_exec.bin chunks (filtered to D2R image range) into the
# active program's memory, then for each target VA:
#   - disassembles the function
#   - decompiles it (full C output)
#   - lists direct callers
#   - lists xrefs to/from
#
# Inputs (env vars):
...
#
# Run via tools/ghidra_run_walk.bat.
...
def list_callers(fn):
    """Find all direct callers of a function via xref."""
    if fn is None:
        return []
    program = currentProgram
...
def main():
    ...
    targets_str = env("D2R_TARGETS", "0x7ff79d451bd0")
    report_path = env("D2R_REPORT",
                      os.path.join(os.path.dirname(bin_path), "ghidra_walk_report.txt"))
    ...
    out_lines = []
    out_lines.append("# D2R request_walk RE report")
    ...
    for va in targets:
        ...
        out_lines.append("--- direct callers (" + str(len(callers)) + ") ---")
        for from_addr, from_fn in callers:
            fn_name = from_fn.getName() if from_fn else "<no fn>"
            fn_entry = str(from_fn.getEntryPoint()) if from_fn else "?"
            out_lines.append("  " + str(from_addr) + "  in " + fn_name + " @ " + fn_entry)
        out_lines.append("")
        ...
        # Also decompile each caller (top 5)
        for i, (from_addr, from_fn) in enumerate(callers[:5]):
            if from_fn is None:
                continue
            out_lines.append("--- caller [" + str(i + 1) + "] decompiled: " +
                             from_fn.getName() + " @ " + str(from_fn.getEntryPoint()) + " ---")
            out_lines.append(decompile_function(from_fn))
            out_lines.append("")
